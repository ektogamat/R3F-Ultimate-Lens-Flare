import { Uniform, Color, Vector3, Object3D } from 'three'
import { BlendFunction, Effect } from 'postprocessing'
import { wrapEffect } from './util.tsx'
import { useRef, forwardRef, useMemo, useLayoutEffect, MutableRefObject } from 'react'
...
const LensFlareShader = {
  ...
  vec2 rtU(vec2 uv, float rtn){return vec2(cos(rtn) * uv.x + sin(rtn) * uv.y,cos(rtn) * uv.y - sin(rtn) * uv.x);}
  vec3 drwF(vec2 p, float intensity, float rnd, float speed, int id){float flhos = (1. / 32.) * float(id) * 0.1;float lingrad = distance(vec2(0.), p);float expg = 1. / exp(lingrad * (fract(rnd) * 0.66 + 0.33));vec3 colgrad = hsv2rgb(vec3( fract( (expg * 8.) + speed * flareSpeed + flhos), pow(1.-abs(expg*2.-1.), 0.45), 20.0 * expg * intensity));float internalStarPoints;if(anamorphic){internalStarPoints = 1.0;} else{internalStarPoints = starPoints;}float blades = length(p * flareShape * sin(internalStarPoints * atan(p.x, p.y)));float comp = pow(1.-satU(blades), ( anamorphic ? 100. : 12.));comp += satU(expg-0.9) * 3.;comp = pow(comp * expg, 8. + (1.-intensity) * 5.);if(flareSpeed > 0.0){return vec3(comp) * colgrad;} else{return vec3(comp) * flareSize * 15.;}}
  float dist(vec3 a, vec3 b) { return abs(a.x - b.x) + abs(a.y - b.y) + abs(a.z - b.z);}
  vec3 satU(vec3 x){return clamp(x, vec3(0.0), vec3(1.0));}
  float glR(vec2 uv, vec2 pos, float zsi){vec2 mni;if(animated){mni = rtU(uv-pos, iTime * 0.1);} else{mni = uv-pos;}float ang = atan(mni.y, mni.x) * (anamorphic ? 1.0 : starPoints);float dist = length(mni);dist = pow(dist, .9);float f0 = 1.0/(length(uv-pos)*(1.0/zsi*16.0)+.2);return f0+f0*(sin((ang))*.2 +.3);}
  float sdHex(vec2 p){p = abs(p);vec2 q = vec2(p.x*2.0*0.5773503, p.y + p.x*0.5773503);return dot(step(q.xy,q.yx), 1.0-q.yx);}
  float fpow(float x, float k){return x > k ? pow((x-k)/(1.0-k),2.0) : 0.0;}
  vec3 rHx(vec2 uv, vec2 p, float s, vec3 col){uv -= p;if (abs(uv.x) < 0.2*s && abs(uv.y) < 0.2*s){return mix(vec3(0),mix(vec3(0),col,0.1 + fpow(length(uv/s),0.1)*10.0),smoothstep(0.0,0.1,sdHex(uv*20.0/s)));}return vec3(0);}
  vec3 mLs(vec2 uv, vec2 pos){vec2 mni = uv-pos;vec2 uvd = uv*(length(uv));float ang = atan(mni.x,mni.y);float f0 = .3/(length(uv-pos)*16.0+1.0);f0 = f0*(sin(niz(sin(ang*3.9-(animated ? iTime : 0.0) * 0.3) * starPoints))*.2 );float f1 = max(0.01-pow(length(uv+1.2*pos),1.9),.0)*7.0;float f2 = max(.9/(10.0+32.0*pow(length(uvd+0.99*pos),2.0)),.0)*0.35;float f22 = max(.9/(11.0+32.0*pow(length(uvd+0.85*pos),2.0)),.0)*0.23;float f23 = max(.9/(12.0+32.0*pow(length(uvd+0.95*pos),2.0)),.0)*0.6;vec2 uvx = mix(uv,uvd, 0.1);float f4 = max(0.01-pow(length(uvx+0.4*pos),2.9),.0)*4.02;float f42 = max(0.0-pow(length(uvx+0.45*pos),2.9),.0)*4.1;float f43 = max(0.01-pow(length(uvx+0.5*pos),2.9),.0)*4.6;uvx = mix(uv,uvd,-.4);float f5 = max(0.01-pow(length(uvx+0.1*pos),5.5),.0)*2.0;float f52 = max(0.01-pow(length(uvx+0.2*pos),5.5),.0)*2.0;float f53 = max(0.01-pow(length(uvx+0.1*pos),5.5),.0)*2.0;uvx = mix(uv,uvd, 2.1);float f6 = max(0.01-pow(length(uvx-0.3*pos),1.61),.0)*3.159;float f62 = max(0.01-pow(length(uvx-0.325*pos),1.614),.0)*3.14;float f63 = max(0.01-pow(length(uvx-0.389*pos),1.623),.0)*3.12;vec3 c = vec3(glR(uv,pos, glareSize));vec2 prot;if(animated){prot = rtU(uv - pos, (iTime * 0.1));} else if(anamorphic){prot = rtU(uv - pos, 1.570796);} else {prot = uv - pos;}c += drwF(prot, (anamorphic ? flareSize * 10. : flareSize), 0.1, iTime, 1);c.r+=f1+f2+f4+f5+f6; c.g+=f1+f22+f42+f52+f62; c.b+=f1+f23+f43+f53+f63;c = c*1.3 * vec3(length(uvd)+.09);c+=vec3(f0);return c;}
  vec3 cc(vec3 clr, float fct,float fct2){float w = clr.x+clr.y+clr.z;return mix(clr,vec3(w)*fct,w*fct2);}
  float rnd(vec2 p){float f = fract(sin(dot(p, vec2(12.1234, 72.8392) )*45123.2));return f;}
  float rnd(float w){float f = fract(sin(w)*1000.);return f;}
  ...
  vec3 drC(vec2 p, float zsi, float decay, vec3 clr, vec3 color2, float dist, vec2 mouse){float l = length(p + mouse*(dist*2.))+zsi/2.;float l2 = length(p + mouse*(dist*4.))+zsi/3.;float c = max(0.01-pow(length(p + mouse*dist), zsi*ghostScale), 0.0)*10.;float c1 = max(0.001-pow(l-0.3, 1./40.)+sin(l*20.), 0.0)*3.;float c2 =  max(0.09/pow(length(p-mouse*dist/.5)*1., .95), 0.0)/20.;float s = max(0.02-pow(rShp(p*5. + mouse*dist*5. + decay, 6) , 1.), 0.0)*1.5;clr = cos(vec3(0.44, .24, .2)*8. + dist*4.)*0.5+.5;vec3 f = c*clr;f += c1*clr;f += c2*clr;f +=  s*clr;return f-0.01;}
  ...
  vec4 txL(sampler2D tex, vec2 texCoord){if(((texCoord.x < 0.) || (texCoord.y < 0.)) || ((texCoord.x > 1.) || (texCoord.y > 1.))){return vec4(0.0);}else{return texture(tex, texCoord); }}
  vec4 txD(sampler2D tex, vec2 texCoord, vec2 dir, vec3 ditn) {return vec4(txL(tex, (texCoord + (dir * ditn.r))).r,txL(tex, (texCoord + (dir * ditn.g))).g,txL(tex, (texCoord + (dir * ditn.b))).b,1.0);}
  vec4 strB(){vec2 aspectTexCoord = vec2(1.0) - (((vTexCoord - vec2(0.5)) * vec2(1.0)) + vec2(0.5)); vec2 texCoord = vec2(1.0) - vTexCoord; vec2 ghvc = (vec2(0.5) - texCoord) * uDispersal - lensPosition; vec2 ghostVecAspectNormalized = normalize(ghvc * vec2(1.0)) * vec2(1.0);vec2 haloVec = normalize(ghvc) * uHaloWidth;vec2 haloVecAspectNormalized = ghostVecAspectNormalized * uHaloWidth;vec2 texelSize = vec2(1.0) / vec2(iResolution.xy);vec3 ditn = vec3(-(texelSize.x * uDistortion), 0.2, texelSize.x * uDistortion);vec4 c = vec4(0.0);for (int i = 0; i < 8; i++) {vec2 offset = texCoord + (ghvc * float(i));c += txD(lensDirtTexture, offset, ghostVecAspectNormalized, ditn) * pow(max(0.0, 1.0 - (length(vec2(0.5) - offset) / length(vec2(0.5)))), 10.0);}vec2 haloOffset = texCoord + haloVecAspectNormalized; return (c * geLC((length(vec2(0.5) - aspectTexCoord) / length(vec2(haloScale))))) +(txD(lensDirtTexture, haloOffset, ghostVecAspectNormalized, ditn) * pow(max(0.0, 1.0 - (length(vec2(0.5) - haloOffset) / length(vec2(0.5)))), 10.0));} 
  void mainImage(vec4 v,vec2 r,out vec4 i){vec2 g=r-.5;g.y*=iResolution.y/iResolution.x;vec2 l=lensPosition*.5;l.y*=iResolution.y/iResolution.x;vec3 f=mLs(g,l)*20.*colorGain/256.;if(aditionalStreaks){vec3 o=vec3(.9,.2,.1),p=vec3(.3,.1,.9);for(float n=0.;n<10.;n++)f+=drC(g,pow(rnd(n*2e3)*2.8,.1)+1.41,0.,o+n,p+n,rnd(n*20.)*3.+.2-.5,lensPosition);}if(secondaryGhosts){vec3 n=vec3(0);n+=rHx(g,-lensPosition*.25,ghostScale*1.4,vec3(.25,.35,0));n+=rHx(g,lensPosition*.25,ghostScale*.5,vec3(1,.5,.5));n+=rHx(g,lensPosition*.1,ghostScale*1.6,vec3(1));n+=rHx(g,lensPosition*1.8,ghostScale*2.,vec3(0,.5,.75));n+=rHx(g,lensPosition*1.25,ghostScale*.8,vec3(1,1,.5));n+=rHx(g,-lensPosition*1.25,ghostScale*5.,vec3(.5,.5,.25));n+=fpow(1.-abs(distance(lensPosition*.8,g)-.7),.985)*colorGain/2100.;f+=n;}if(starBurst){vTexCoord=g+.5;vec4 n=geLD(g);float o=1.-clamp(uBrightDark,0.,.5)*2.;n+=mix(n,pow(n*2.,vec4(2))*.5,o);float s=(g.x+g.y)*(1./6.);vec2 d=mat2(cos(s),-sin(s),sin(s),cos(s))*vTexCoord;n+=geLS(d)*2.;f+=clamp(n.xyz*strB().xyz,.01,1.);}i=enabled?vec4(mix(f,vec3(0),opacity)+v.xyz,v.w):vec4(v);}
`
}
